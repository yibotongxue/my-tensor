...
TEST(SoftmaxTest, ForwardCPUTest) {
  ...
  bottom->SetCPUData(bottom_data);
  softmax->ForwardCPU({bottom}, {top});
  std::vector<float> actual(top->GetCPUData().begin(), top->GetCPUData().end());
