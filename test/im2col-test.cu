...
TEST(Im2colTest, test_one_channel) {
  ...
  im_tensor->SetCPUData(data);

  const std::vector<int> col_shape = {1, 48, 9};
  auto col_tensor = std::make_shared<my_tensor::Tensor<>>(col_shape);
  ASSERT_NO_THROW(my_tensor::im2col_cpu(im_tensor->GetCPUDataPtr(), 1, 6, 8, 3, 3, col_tensor->GetCPUDataPtr()));
  ...
  const std::vector<float> actual(col_tensor->GetCPUData().begin(), col_tensor->GetCPUData().end());
