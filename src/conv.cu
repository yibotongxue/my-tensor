...
void Convolution<T>::BackwardCPU(const TensorPtr<T>& top, TensorPtr<T>& bottom) {
  ...
  // top = kernel * temp
  // [n * c_out * im_size] = [(n *) c_out * (c_in * kernel_size)] * [n * (c_in * kernel_size) * im_size]
  // partial temp
  for (int t = 0; t < n; t++) {
...
void Convolution<T>::BackwardGPU(const TensorPtr<T>& top, TensorPtr<T>& bottom) {
  ...
  transpose_matmul(this->params_[0]->GetGPUDataPtr(), top->GetGPUDiffPtr(),
      temp_tensor->GetGPUDiffPtr(), c_in * kernel_size, c_out, im_size, n, 1);
  Col2im_GPU(n, temp_tensor->GetGPUDiffPtr(), c_in, height, width,
      kernel_height, kernel_width, bottom->GetGPUDiffPtr());
  // partial kernel
  std::vector<int> batch_kernel_shape {n, c_out, c_in, kernel_height, kernel_width};
  ...
  matmul_transpose(top->GetGPUDiffPtr(), temp_tensor->GetGPUDataPtr(),
      batch_kernel->GetGPUDiffPtr(), c_out, im_size, c_in * kernel_size, n);
  col_sum(batch_kernel->GetGPUDiffPtr(), this->params_[0]->GetGPUDiffPtr(),
      n, c_out * c_in * kernel_height * kernel_width);
}
